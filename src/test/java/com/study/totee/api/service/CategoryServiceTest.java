...
import static com.study.totee.api.util.EntityFactory.category;
...
public class CategoryServiceTest {
    ...
    public void 카테고리등록실패_이미존재(){
        // given
        doReturn(CategoryEntity.builder().build()).when(categoryRepository).findByCategoryName("TEST");
    ...
    public void 카테고리등록성공() throws IOException {
        // given
        doReturn(null).when(categoryRepository).findByCategoryName("TEST");
        ...
        //verify
        verify(categoryRepository, times(1)).findByCategoryName("TEST");
    ...
    public void 카테고리목록조회() {
        ...
        // when
        final List<CategoryResponseDto> result = target.categoryEntityList();
    ...
    public void 카테고리삭제성공() {
        // given
        final CategoryEntity categoryEntity = category();
